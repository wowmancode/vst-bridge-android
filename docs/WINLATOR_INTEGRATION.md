...
The bridge copies the host and plug-in into the first container, then starts the Winlator display activity with this command. VST3 editor hosting and a real-time Android DAW transport remain future work.

## Packaging boundary

The existing `vst-bridge-runtime-debug-apk` already embeds the x86-64 Windows host.
Wine, Box64, the root filesystem, X server, audio libraries, and their pinned source are now included in the Android build. The Windows host remains a downstream CI artifact so the runtime APK always contains a freshly tested host.
